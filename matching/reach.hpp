...
void dfs_in(node &u) {
  ...
  if (u.N_I_SZ == 0) {
    ...
    for (int i = 0; i < u.N_I_SZ; i++) {
      ...
      if (v.N_I_SZ == 0) {
        int hu = v.h_in;
        u.L_in[(hu >> 5) % KReach] |= 1 << (hu & 31);
      } else {
        for (int j = 0; j < KReach; j++) {
          u.L_in[j] |= v.L_in[j];
        }
      }
    }
...
int run_queries() {
  ...
  for (auto & querie : queries) {
    vis_cur++;
    int result = reach(querie.first.first, querie.first.second);
    if (querie.second == -1 || querie.second == result) {
      querie.second = result;
      if (result == 1)
        res++;
    } else {
...
int reach(std::vector<std::vector<int>>& N_O, std::vector<std::vector<int>>& N_I, int n, std::string s) {
  using namespace bs;
